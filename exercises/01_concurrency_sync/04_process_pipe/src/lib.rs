...
pub fn pipe_through_cat(input: &str) -> String {
    ...
    child.stdout.take().expect("Failed to open stdout").read_to_end(&mut output).expect("Failed to read from stdout");
...
pub fn get_exit_code(command: &str) -> i32 {
    ...
    child.code().unwrap_or(-1) // Return -1 if process was terminated by signal or other means
}
...
pub fn pipe_through_grep(pattern: &str, input: &str) -> String {
    ...
    } // Drop stdin here to close the write end
