...
impl Scheduler {
    ...
    pub fn spawn(&mut self, entry: extern "C" fn()) {
        ...
        ctx.sp = (stack_top - 16) as u64 & !15; // Align to 16 bytes and leave headroom
    ...
    fn schedule_next(&mut self) {
        let start = (self.current + 1) % self.threads.len();
        let next = (start..self.threads.len())
            .chain(0..start)
            .find(|&i| self.threads[i].state == ThreadState::Ready)
            .unwrap_or(self.current); // No ready thread, stay on current

        if next != self.current {
            let current = self.current;

            //split_at_mut
            let (left, right) = self.threads.split_at_mut(next.max(current));
            let (current_thread, next_thread) = if current < next {
                (&mut left[current], &mut right[0])
            } else {
                (&mut right[0], &mut left[next])
            };
            ...
            self.current = next;
            unsafe {
                switch_context(&mut current_thread.ctx, &next_thread.ctx);
            }
        }
    }
}
