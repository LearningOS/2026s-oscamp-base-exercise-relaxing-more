...
impl Tlb {
    ...
    pub fn lookup(&mut self, vpn: u64, asid: u16) -> Option<u64> {
        ...
        if self.entries.iter().any(|entry| entry.valid && entry.vpn == vpn && entry.asid == asid) {
            self.stats.hits += 1;
            self.entries.iter().find(|entry| entry.valid && entry.vpn == vpn && entry.asid == asid).map(|entry| entry.ppn)
        } else {
            self.stats.misses += 1;
            None
        }
...
impl Mmu {
    ...
    pub fn translate(&mut self, vpn: u64) -> Option<u64> {
        ...
        // 2. TLB 未命中，查页表
        if let Some((_, mapping)) = self.page_table.iter().find(|(asid, mapping)| *asid == self.current_asid && mapping.vpn == vpn) {
            // 页表命中，回填 TLB
